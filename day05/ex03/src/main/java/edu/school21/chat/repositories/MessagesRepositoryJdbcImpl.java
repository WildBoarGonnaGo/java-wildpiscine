...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
	...
	public void					update(Message message) {
		Long			id;
		ResultSet		evilQuery;
		Statement		updateWarlock;
		java.util.Date	parsedDate;
		Timestamp		timestamp = null;

		if (message == null || message.getId() == null) {
			System.err.println("Error: there is no value is message table to update");
			return ;
		}
		if (message.getId() == null) {
			System.err.println("Error: the message id is null, so message will be created");
			save(message);
			return ;
		}
		id = message.getId();
		try {
			evilQuery = queryWarlock.executeQuery("SELECT COUNT(*) FROM message;");
			if (evilQuery.next()) {
				if (id == 0 || id > evilQuery.getLong("count"))
					throw new SQLException("Message entity can't be updated");
			}
			updateWarlock = dbConn.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
			evilQuery = updateWarlock.executeQuery("SELECT * FROM message WHERE idmessage = "
				+ message.getId() + ';');
			if (evilQuery.next()) {
				evilQuery.updateLong("author", message.getAuthor().getId());
				evilQuery.updateLong("room", message.getRoom().getId());
				evilQuery.updateString("text", message.getText());
			...
			System.err.println("Message entity can't be update");
		}
	}
}
